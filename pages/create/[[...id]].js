...
const ProductsManager = () => {
  ...
  useEffect(() => {
    if (id) {
      ...
      getData(`product/${id}`).then((res) => {
        setProduct(res.product), setImages(res.product.images);
      });
    } else {
      setOnEdit(false);
      setProduct(initialState), setImages([]);
    }
  }, [id]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (auth.user.role !== 'admin')
      return dispatch({
        type: 'NOTIFY',
        payload: { error: 'Authenication is  not valid.' },
      });
      ...
      <form className="row" onSubmit={handleSubmit}>
        ...
        <div className="col-md-6 my-4">
          <div className="input-group mb-3">
            <div className="input-group-preerend">
          ...
          <div className="row img-up mx-0">
            {images.map((img, index) => (
              <div key={index} className="file_img">
                ...
                />
                <span onClick={() => deleImage(index)}>X</span>
              </div>
            ))}
          </div>
        </div>
